...
async def cz_auth_page(
    ...
    # Проверяем статус токена для отображения
    has_token = False
    expires = None
...
from fastapi.responses import HTMLResponse, RedirectResponse
from fastapi.templating import Jinja2Templates
from app.core.dependencies import get_current_admin, get_current_client, get_current_user   # если get_current_user всё же нужен, но его нет, лучше убрать
from app.core.config import settings
from app.models.user import User, UserRole
from app.models.client import Client
from app.models.client_address import ClientAddress
...
# ===== НОВАЯ СТРАНИЦА ДЛЯ КЛИЕНТОВ =====
@router.get("/cz-auth", response_class=HTMLResponse)
async def cz_auth_page(
    request: Request,
    current_user: User = Depends(get_current_client),   # используем get_current_client
    db: AsyncSession = Depends(get_db)
):
    """
    Страница получения токена Честного знака.
